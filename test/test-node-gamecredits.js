...
var gamecredits = require('../lib/gamecredits')(options);
...
exports.get = function (test) {
	...
	for(var propt in options){
		test.deepEqual(gamecredits.get(''+options_keys[idx]), options[''+propt]);
		idx ++;
	}
...
exports.set = function (test) {
	...
	for(var propt in new_options){
		gamecredits.set(''+options_keys[idx], new_options[ ''+options_keys[idx] ]);
		test.deepEqual(gamecredits.get(''+options_keys[idx]), new_options[''+propt]);
		idx ++;
	}
	test.done();
}
 ...
 * Either run gamecreditsd directly or run gamecredits-qt with the -server
 * command line option. Make sure you have a ~/.gamecredits/gamecredits.conf
 * with rpcuser and rpcpassword config values filled out. Note that
 * newer versions of gamecredits (1.5 and above) don't allow identical
...
exports.commands_noAuth = {		
	...
	// 			or add "gen=1" to the bottom of your gamecredits.conf file
	getBalance: function(test){
		var curr_balance;
		gamecredits.getBalance(function(err, balance) {
			test.ifError(err);
		  if (err) {
	...
	getGenerate: function(test){
		test.expect(2);
		gamecredits.setGenerate(true,1);	
		test.equal(gamecredits.getGenerate(), true);

		gamecredits.setGenerate(false,1);	
		test.equal(gamecredits.getGenerate(), false);
		test.done();
	},
	getreceived_: function(test){
		var amount= 0.0001;
		//gamecredits.setAccount()
		sendfrom("testnet_user", gamecredits.getaccountaddress('testnet_user'),amount, function(err,addr){
			test.equal(getreceivedbyaccount('testnet_user', amount);
			test.equal( getreceivedbyaddress( gamecredits.getaccountaddress('testnet_user') ), amount);
			test.done();
...
exports.commands_Auth = {		
	...
	setUp: function () {
		gamecredits.auth('testnet_user', 'testnet_pass');
	}
