# A Node.js GameCredits Client!

![GameCredits](https://raw.githubusercontent.com/Gamecredits-Universum/node-gamecredits/master/node-gamecredits.png)

node-gamecredits is a GameCredits client for Node.js.

The purpose of this repository is:

* Provide a one-stop resource for the Node.js developer to get started with GameCredits integration.
* Promote Node.js development of GameCredits web apps.

## Dependencies

You'll need a running instance of [GameCreditsd](https://github.com/Gamecredits-Universum/GameCredits-Core) to connect with.

Then, install the node-gamecredits NPM package.

`npm install node-gamecredits`

## Examples

```js
var gamecredits = require('node-gamecredits')()

gamecredits.auth('myusername', 'mypassword')

gamecredits.getDifficulty(function() {
...
```js
gamecredits.getNewAddress(function(err, address) {
...
```js
gamecredits.exec('getNewAddress')

gamecredits.exec('getbalance', function(err, balance) {
...
```js
gamecredits.set('host', '127.0.0.1')
```
...
var gamecredits = require('gamecredits')({
    user:'user'
})

gamecredits.set('pass', 'password')
gamecredits.set({port:40001})
...
You may pass an optional function `passphrasecallback` to the `node-gamecredits` initialization function to manage wallet unlocks. `passphrasecallback` should be a function accepting three arguments:
...
By default `gamecreditsd` exposes its JSON-RPC interface via HTTP; that is, all RPC commands are transmitted in plain text across the network! To secure the JSON-RPC channel you can supply `gamecreditsd` with a self-signed SSL certificate and an associated private key to enable HTTPS. For example, in your `gamecredits.conf`:

    rpcssl=1
    rpcsslcertificatechainfile=/etc/ssl/certs/gamecreditsd.crt
    rpcsslprivatekeyfile=/etc/ssl/private/gamecreditsd.pem

In order to securely access an SSL encrypted JSON-RPC interface you need a copy of the self-signed certificate from the server: in this case `gamecreditsd.crt`. Pass your self-signed certificate in the `ca` option and set `https: true` and node-gamecredits is secured!
    
```js
var fs = require('fs')

var ca = fs.readFileSync('gamecreditsd.crt')
...
nodeunit test/test-node-gamecredits.js
```
